...
#ifdef WIN32
#include "crpi/crpi.h"
#include "crpi/crpi_robot.h"
#include "ulapi/ulapi.h"
...
namespace MotionPrims
{
  ...
  public:
    ...
    //! @param zoff  Z axis offset
    //! @param speed Speed of motions at which the search is performed
    //!
    //! @return CANON_SUCCESS if the search routine was added successfully, CANON_FAILURE otherwise
    //!
    ...
    //!
    //! @param rType   Specified return type upon meeting the termination condition
    ...
    //! @brief Add an external/internal signal termination condition
    //!
    //! @param rType  Specified return type upon meeting the termination condition
    ...
    //! @brief Add a distance threshold termination condition
    //!
    //! @param rType Specified return type upon meeting the termination condition
    //! @param x     X axis max travel distance threshold
    //! @param y     Y axis max travel distance threshold
    //! @param z     Z axis max travel distance threshold
    //! @param total Euclidean max travel distance threshold
    //!
    //! @return CANON_SUCCESS if the termination condition was added successfully, CANON_FAILURE otherwise
    //!
    CanonReturn AddTerminatorDistance (CanonReturn rType, double x, double y, double z, double total);

    //! @brief Add a search repetition termination condition
    //!
    //! @param rType Specified return type upon meeting the termination condition
    //! @param reps  Maximum number of repetitions before declaring success/failure
    //!
    //! @return CANON_SUCCESS if the termination condition was added successfully, CANON_FAILURE otherwise
    //!
    ...
    //! @brief Execut a single time step of the assembly and generate a new target robot pose
    //!
    ...
    //! @param ios     The current I/O values read by the system
    //!
    //! @return The current search status, or the status of the completed search based on the defined
  ...
  private:
    ...
    //! @brief Add a search primitive to the sequential list of search primitives
    //!
    //! @param aP Defined search primitive to be added to the search sequence
    //!
    //! @return CANON_SUCCESS if search primitives added successfully, CANON_FAILURE otherwise
    //!
    CanonReturn AddSearch (assemblyParams &aP);

    //! @brief Add a termination condition to the list of condition tests
    //!
    //! @param tP Defined termination conditions to be added to the condition tests
    //!
    //! @return CANON_SUCCESS if termination conditions successfully added, CANON_FAILURE otherwise
    //!
