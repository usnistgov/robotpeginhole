...
#include "crpi/crpi.h"
#include "crpi/crpi_xml.h"
#include "crpi/crpi_robot_xml.h"
#include "crpi/vector.h"
...
namespace crpi_robot
{
  ...
  public:
    ...
    //!         not accepted, and FAILURE if the command is accepted but not executed successfully
    //!
    CanonReturn MoveStraightTo (robotPose &pose, bool useBlocking = true);
    ...
    //!         not accepted, and FAILURE if the command is accepted but not executed successfully
    //!
    CanonReturn MoveTo (robotPose &pose, bool useBlocking = true);
    ...
    //! @brief Calculate a pose to point the end effector toward the pose specified
    //!
    //! @param to  Target location that the robot is to point toward
    //! @param out The output pose for the robot that points at the target location
    //! 
    //! @return SUCCESS if command is accepted and is executed successfully, REJECT if the command is
    //!         not accepted, and FAILURE if the command is accepted but not executed successfully
    //!
    CanonReturn Orient (robotPose &to, robotPose *out);
