...
namespace crpi_robot
{
	struct CRobotImpl
	{
		...
		tf::Pose addGrasped(tf::Pose pose);
		tf::Pose removeGrasped(tf::Pose pose);
		 ...
		 */
		CanonReturn move(moveit_msgs::RobotTrajectory trajectory);

		CanonReturn gzMovePt(trajectory_msgs::JointTrajectoryPoint pt);

		CanonReturn moveTraj(tf::Pose curpose,
					 tf::Pose rbtFinal,
					 double velscale);
	...
	public:
		...
		~CWm();
		void stop();
		bool isReadModels() { return bReadAllInstances;}
		...
		bool getInstance(std::string name, CWm &wm);
		std::string toString();
